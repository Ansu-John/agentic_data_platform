...
class IcebergIngestionPipeline:
    ...
    def read_source_data(self) -> DataFrame | None:
        ...
        try:
            ...
            if df.isEmpty():
                logger.warning(f"Source path {self.source_path} resulted in an empty DataFrame. "
                               f"Aborting.")
                return None
    ...
    def write_to_iceberg(self, df: DataFrame) -> None:
        ...
        if not table_exists:
            logger.info(f"Table does not exist. Creating {self.iceberg_table_identifier} "
                        f"at {self.s3_location}")
...
def parse_arguments()-> argparse.Namespace:
    ...
    parser.add_argument("--merge-key", required=True, help="Primary key for UPSERT/MERGE operation")
